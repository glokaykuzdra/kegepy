...
Изображение `image` откроется отдельным окном. Поддерживаются форматы ``.png``, ``.jpg`` и ``.gif``. Однако `.web_open()` не работает корректно со всеми изображениями. Файл изображения можно загрузить в папку с модулем kegepy или на указанный путь: `image.download(path="your_path"`. Формат загруженного файла `.png`.
